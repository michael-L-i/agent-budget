...
3. For this session, run the helper's `set` command with the JSON on stdin. For
...
4. Only say **Budget set** after the helper reports `active`. Briefly state the
...
Percentage budgets require CodexBar's CLI reader to work for the same local
account. A missing or unidentifiable account must remain an error.
